...
function Particles() {
  ...
  const positions = useMemo(() => {
    ...
    for (let i = 0; i < 300 * 3; i++) arr[i] = (Math.random() - 0.5) * 15;
    return arr;
  }, []);
  ...
  return (
    <points ref={points}>
      ...
      <pointsMaterial size={0.02} color="#FF9933" transparent opacity={0.5} sizeAttenuation />
    </points>
  );
}
    ...
    const timeout = setTimeout(
      () => {
        if (!deleting) {
          ...
          if (text.length === 0) {
            setDeleting(false);
            setCurrentIndex((p) => (p + 1) % typewriterStrings.length);
          }
        }
      },
      deleting ? 50 : 100
